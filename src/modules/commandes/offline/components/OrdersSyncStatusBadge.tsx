...
import { triggerOrdersSync } from "../services/ordersSyncActions";
import { useOrdersSyncStore } from "../store/ordersSyncStore";

export function OrdersSyncStatusBadge() {
  const queryClient = useQueryClient();
  const { isSyncing, pendingCount, lastError } = useOrdersSyncStore();
    ...
    <div className="flex items-center gap-2">
      {pendingCount > 0 ? (
        <span className="rounded-full bg-warning-50 px-2.5 py-1 text-xs font-medium text-warning-700 dark:bg-warning-500/10 dark:text-warning-300">
          Commandes · {pendingCount} en attente
        </span>
      ) : null}

      {isSyncing ? (
        <span className="text-xs text-gray-500">Sync. commandes...</span>
      ) : null}

      {pendingCount > 0 || lastError ? (
        <button
          type="button"
          onClick={() => void triggerOrdersSync(queryClient)}
