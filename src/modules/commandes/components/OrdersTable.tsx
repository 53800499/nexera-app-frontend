"use client";

import Link from "next/link";
import { DataTable, type DataTableColumn } from "@/shared/components/table";
import { EmptyState } from "@/shared/components/feedback";
import type { OrderSummary } from "../types/order.types";
import { formatMoney } from "@/modules/devis/utils/quotationCalculations";
import { OrderStatusBadge } from "./OrderStatusBadge";
import { normalizeOrderStatus } from "../utils/orderLabels";
import { canEditOrder } from "../utils/orderStatusRules";

type Props = {
  orders: OrderSummary[];
  canManage: boolean;
};

function formatDate(value: string) {
  return new Date(value).toLocaleDateString("fr-FR");
}

export function OrdersTable({ orders, canManage }: Props) {
  const columns: DataTableColumn<OrderSummary>[] = [
    {
      key: "number",
      header: "N° BC",
      render: (row) => (
        <p className="font-medium text-gray-800 dark:text-white/90">
          {row.number}
        </p>
      ),
    },
    {
      key: "client",
      header: "Client",
      render: (row) => (
        <div>
          <p className="text-sm">{row.client?.companyName ?? "—"}</p>
          <p className="text-xs text-gray-500">{row.client?.code ?? ""}</p>
        </div>
      ),
    },
    {
      key: "quotation",
      header: "Devis source",
      render: (row) =>
        row.quotation ? (
          <Link
            href={`/devis/${row.quotation.id}`}
            className="text-sm text-brand-600 hover:underline"
          >
            {row.quotation.number}
          </Link>
        ) : (
          "—"
        ),
    },
    {
      key: "date",
      header: "Date",
      render: (row) => formatDate(row.issueDate),
    },
    {
      key: "amount",
      header: "Montant TTC",
      render: (row) => formatMoney(row.totalTtc, row.currency),
    },
    {
      key: "billing",
      header: "Facturé",
      render: (row) => `${row.billing.billingProgressPct.toFixed(0)} %`,
    },
    {
      key: "status",
      header: "Statut",
      render: (row) => (
        <OrderStatusBadge status={normalizeOrderStatus(row.status)} />
      ),
    },
    {
      key: "actions",
      header: "Actions",
      render: (row) => (
        <div className="flex items-center gap-2">
          <Link
            href={`/commandes/${row.id}`}
            className="rounded border border-gray-300 px-2 py-1 text-xs hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
          >
            Détail
          </Link>
          {canManage && canEditOrder(row.status) ? (
            <Link
              href={`/commandes/${row.id}/modifier`}
              className="rounded border border-brand-300 px-2 py-1 text-xs text-brand-600 hover:bg-brand-50 dark:border-brand-500/40 dark:text-brand-400"
            >
              Modifier
            </Link>
          ) : null}
        </div>
      ),
    },
  ];

  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
      <div className="max-w-full overflow-x-auto">
        <div className="min-w-[1000px]">
          <DataTable<OrderSummary>
            data={orders}
            columns={columns}
            rowKey={(row) => row.id}
            emptyState={
              <EmptyState
                title="Aucun bon de commande"
                description="Créez un BC manuellement ou convertissez un devis accepté."
                className="border-0 bg-transparent py-8"
                action={
                  canManage ? (
                    <Link
                      href="/commandes/nouveau"
                      className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600"
                    >
                      Nouveau BC
                    </Link>
                  ) : undefined
                }
              />
            }
          />
        </div>
      </div>
    </div>
  );
}
