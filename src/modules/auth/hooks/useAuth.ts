...
export function useAuth() {
  ...
  const login = useCallback(
    async (credentials: LoginCredentials) => {
      ...
      try {
        ...
        const userName = session.user.firstName ? ` ${session.user.firstName}` : "";
        await showResult({
          variant: "brand",
          title: "Connexion réussie !",
          message: `Ravi de vous revoir${userName} ! Redirection vers votre espace de travail...`,
          closeLabel: "Accéder au tableau de bord",
          autoCloseMs: 2500,
        });
    ...
    [clearError, loginMutation, setError, setSessionReady, setUser, showResult],
  );
  ...
  const register = useCallback(
    async (data: RegisterDirigeantData) => {
      ...
      try {
        ...
        await showResult({
          ...
          message: `Bienvenue${userName} ! Votre compte NEXERA ERP a été configuré avec succès.`,
          closeLabel: "Accéder à mon espace",
          autoCloseMs: 2500,
        });
        window.location.assign(redirectPath);
        return redirectPath;
    ...
    [clearError, registerMutation, setError, setSessionReady, setUser, showResult],
  );
