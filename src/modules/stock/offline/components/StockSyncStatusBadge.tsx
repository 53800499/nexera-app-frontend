...
import { clearStockOfflineIfOnline } from "../utils/stockOfflineState";
import { triggerStockSync } from "../services/stockSyncActions";
import { useStockSyncStore } from "../store/stockSyncStore";

export function StockSyncStatusBadge() {
  const queryClient = useQueryClient();
  const { isOffline, isSyncing, pendingCount, lastError } = useStockSyncStore();
    ...
    <div className="flex items-center gap-2">
      {isOffline ? (
        <span className="rounded-full bg-error-50 px-2.5 py-1 text-xs font-medium text-error-700 dark:bg-error-500/10 dark:text-error-400">
          {showStaleOffline ? "API indispo (Stock)" : "Stock hors-ligne"}
        </span>
      ) : null}
      ...
      {isSyncing ? (
        <span className="text-xs text-gray-500">Synchronisation stock...</span>
      ) : null}

      {!isOffline && (pendingCount > 0 || lastError) ? (
        <button
          type="button"
          onClick={() => void triggerStockSync(queryClient)}
          className="rounded-lg border border-gray-300 px-2 py-1 text-xs font-medium hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
        >
          Synchroniser
        </button>
