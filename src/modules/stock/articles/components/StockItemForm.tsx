...
export function StockItemForm({
  ...
  const initialValues = useMemo<StockItemFormValues>(
    () => ({
      ...
      expiryAlertDays: defaultValues?.expiryAlertDays ?? 30,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {trackExpiry ? (
            <div>
              <Label>Seuil alerte péremption (jours)</Label>
              <Input
                type="number"
                step="1"
                min="0"
                {...register("expiryAlertDays", { valueAsNumber: true })}
                error={Boolean(errors.expiryAlertDays)}
                hint={
                  errors.expiryAlertDays?.message ??
                  "Alerte si (date péremption − aujourd’hui) ≤ ce seuil"
                }
              />
            </div>
          ) : null}
